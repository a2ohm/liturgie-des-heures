...
if (office === 'lectures') {
  ...
  if (office === 'laudes') {
	...
	if (psalmodie != "") {
		$('#la_psalmodie_inv').html('<img src="psaumes/psalmodie/' + psalmodie + '/psaume94_psalmodie.svg" onerror="this.style.display=\'none\'">')
	}
	...
	$('#la_select_inv').bind('change', function (event) {
		...
		if (psalmodie != "") {
			$('#la_psalmodie_inv').html('<img src="psaumes/psalmodie/' + psalmodie + '/psaume' + valeur + '_psalmodie.svg" onerror="this.style.display=\'none\'">')
		}
	});
  }
...
function Mode_nuit() {
	...
	if (mode_nuit) {
		// Pour les écrans AMOLED, un arrière plan vraiment noir peut être
		// plus confortable, et cela économise de la batterie
